...
// because foo is local and is only accesable in the for
...
// because there is both a global x var and a refrence to x in the for statement where as foo was only in the for
...
func greeting(greeting: String?, name: String) {
    guard let successfulGreeting = greeting else {
        print("Hello \(name)")
        return
    }
    print("\(successfulGreeting), \(name)")
}
