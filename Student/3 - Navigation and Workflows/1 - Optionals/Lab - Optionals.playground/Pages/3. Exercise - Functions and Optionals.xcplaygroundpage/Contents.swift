...
func checkAge() {
    if let age = Int(userInputAge) {
        ...
        if unwrappedAge >= 18 {
            ...
            print("you are too young to use this come back when your older")
        }
    } else {
...
func makeSale(userInput: String) {
    if let selectedItem = stock[userInput], let selectedPrice = prices[userInput] {
        if selectedItem <= 0 {
            print("Sorry the item you have selected is out of stock check back another time")
        } else {
            print("the price for this item is \(selectedPrice), there are currently \(selectedItem) in stock")
        }
    }
}

makeSale(userInput: "Chips")
makeSale(userInput: "BADDATA")
